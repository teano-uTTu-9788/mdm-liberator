...
OVERALL_LOCAL_MANAGEMENT_RISK="clear_local_signals"
...
mark_detected() {
  ...
  OVERALL_LOCAL_MANAGEMENT_RISK="action_needed"
...
mark_inconclusive() {
  ...
  if [[ "${OVERALL_LOCAL_MANAGEMENT_RISK}" != "action_needed" ]]; then
    OVERALL_LOCAL_MANAGEMENT_RISK="inconclusive"
  fi
...
echo -e "  Overall local risk         : ${BOLD}${OVERALL_LOCAL_MANAGEMENT_RISK}${RESET}"
...
write_text_report() {
  local path="$1"
  {
    ...
    echo "  Overall local management risk: ${OVERALL_LOCAL_MANAGEMENT_RISK}"
...
write_json_report() {
  local path="$1"
  {
    ...
    echo "  \"overall_local_management_risk\": \"${OVERALL_LOCAL_MANAGEMENT_RISK}\","
    echo "  \"permissions\": \"${PERMISSIONS}\","
